...
void TestInvalidParameterHandler(const wchar_t* expression,
                                 ...
                                 unsigned int line,
                                 uintptr_t pReserved) {
  BLOG(LS_ERROR) << "InvalidParameter Handler called.  Exiting.";
  BLOG(LS_ERROR) << expression << std::endl << function << std::endl << file
                << std::endl << line;
  exit(1);
}
void TestPureCallHandler() {
  BLOG(LS_ERROR) << "Purecall Handler called.  Exiting.";
  exit(1);
}
int TestCrtReportHandler(int report_type, char* msg, int* retval) {
    BLOG(LS_ERROR) << "CrtReport Handler called...";
    BLOG(LS_ERROR) << msg;
